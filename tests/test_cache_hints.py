...
from eth_library_mcp.server import CACHE_HINTS, LIST_CACHE_TTL_MS, mcp
...
async def test_der_inhalt_einer_ressource_traegt_keinen_frischehinweis() -> None:
    ...
    """
    async with Client(mcp) as client:
        result = await client.read_resource("eth://resource-types")
...
def test_die_readme_nennt_die_sdk_anforderung_aus_pyproject() -> None:
    """Die README nannte `mcp[cli]>=1.0.0,<2.0.0` — eine Range, die genau die
    Version ausschliesst, die `pyproject.toml` verlangt (`>=2.0.0,<3`). Wer die
    README als Installationsanweisung las, bekam ein SDK, mit dem dieser Server
    nicht laeuft. Verglichen hat die beiden nichts; hier steht der Vergleich."""
    import pathlib
    import tomllib
